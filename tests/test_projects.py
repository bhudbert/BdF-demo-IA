...
def test_create_project_with_all_roles(client: TestClient):
    ...
    # Créer les personnes
    chef = client.post("/api/v1/persons/", json={"last_name": "Chef", "first_name": "Alice"})
    ligne = client.post("/api/v1/persons/", json={"last_name": "Ligne", "first_name": "Bob"})
    dev = client.post("/api/v1/persons/", json={"last_name": "Dev", "first_name": "Charlie"})
...
def test_read_projects(client: TestClient):
    ...
    # Créer une personne
    person = client.post("/api/v1/persons/", json={"last_name": "Dev", "first_name": "Test"})
    dev_id = person.json()["id"]

    # Créer des projets
    client.post("/api/v1/projects/", json={"name": "Projet 1", "lead_developer_id": dev_id})
    client.post("/api/v1/projects/", json={"name": "Projet 2", "lead_developer_id": dev_id})
