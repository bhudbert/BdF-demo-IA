...
def test_create_person_duplicate(client: TestClient):
    ...
    # Première création
    response1 = client.post("/persons/", json=person_data)
    assert response1.status_code == 201

    # Deuxième création (devrait échouer)
    response2 = client.post("/persons/", json=person_data)
...
def test_read_persons(client: TestClient):
    ...
    client.post("/persons/", json={"last_name": "Dupont", "first_name": "Jean"})
    client.post("/persons/", json={"last_name": "Martin", "first_name": "Sophie"})
